...
`refresh_interval_ms` 控制看板运行时的受控数据刷新：`0` 为关闭，其他值必须在 1000–3600000 毫秒之间，默认 5000。前端通过已发布数据模型轮询刷新，不直接访问设备协议驱动。可视化编辑器支持组件库、拖拽、缩放和属性编辑，并保留 JSON 作为高级编排入口；图表运行时提供柱状、折线和饼图 SVG 渲染。导入导出仅接受 iView 自身的 `iview-dashboard` v1 包，并在保存前重新校验组件绑定、变量、条件样式和交互定义。
