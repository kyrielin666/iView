...
项目目标是将设备通信、数据采集、多源数据集、生产分析与低代码看板组合成一套容易部署、容易二次开发的 JVM 工程底座。

## 项目特点

- 前后端独立部署：JVM 后端与纯 Web 前端完全分离。
- 原生 JVM 协议驱动：已有 Modbus TCP/RTU、三菱 MC 3E/4E/A1E、OMRON FINS/TCP 与 FINS/UDP、Siemens S7 和 OPC UA 共九种驱动实现；代码测试通过不代表真实设备验收已经完成。
...
- 管理安全：管理员维护用户、角色、接口权限与设备数据范围，用户停用即时失效，并记录管理操作审计。
- 本地运维：H2 部署支持管理员备份、SHA-256 校验、保留策略与维护窗口恢复。
...
以下内容表示代码和自动化测试中已有实现，不等同于旧系统全部功能已完成等价迁移，也不替代真实 PLC、CNC 与数据库环境的连续运行验收。
...
- Modbus TCP/RTU、三菱 MC 3E/4E/A1E、OMRON FINS/TCP 与 FINS/UDP、Siemens S7 和 OPC UA 驱动及自动化测试
- 实时值、历史采样、设备统计和采集调度
...
- PostgreSQL、MongoDB、HTTP JSON、CSV/JSON 文件数据源，支持连接测试、结构发现和统一预览
- 数据集 CRUD、目录、复制、移动、预览、节点/边字段血缘、Explain 与 CSV 导出
- 数据模型字段同步、漂移检测、发布前差异与破坏性变更检查、乐观版本发布和安全变量查询
- 看板草稿、文件夹、发布快照、可视化编排、受控自动刷新与模板导入导出
- 指标、仪表盘、进度条、状态灯、储罐、阀门、电机、流动管线、报警牌、时钟、普通/轮播表格、SVG 柱状/折线/饼图/漏斗/雷达/散点图、视频、设备坐标图、文本、图片、iframe、装饰、容器和元组等组件
...
modules/protocol-omron-fins      原生 JVM OMRON FINS/TCP 驱动
...
- 完善部门组织和自定义菜单权限
- 扩充组件库（旧看板项目约有百余组件入口），完善图层、组合、撤销重做与响应式发布运行时
...
- [OMRON FINS/TCP](docs/protocols/omron-fins-tcp.md)
